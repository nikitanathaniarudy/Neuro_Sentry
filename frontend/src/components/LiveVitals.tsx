...
import { connectLiveState, LiveVitals as LiveVitalsType, GeminiReport, connectAudioStream } from "../ws";
...
const LiveVitals: React.FC = () => {
    ...
    const audioWs = useRef<{ send: (data: ArrayBuffer) => void; close: () => void } | null>(null);
    ...
    useEffect(() => {
        const cleanup = connectLiveState({
            onMessage: (msg) => {
                if (msg.type === "live") {
                    const currentCount = msg.data.session_packet_count;

                    // Start audio recording when iOS session starts (packet count goes to 1)
                    if (currentCount === 1 || (currentCount < 10 && currentCount < prevPacketCount.current)) {
                        console.log('[LiveVitals] Detected session start, starting audio...');
                        startAudioRecording();
                    }

                    // Aggressive reset: If packet count is low (start of session) or drops, clear report
                    if (currentCount < 10 || currentCount < prevPacketCount.current) {
                        setReport(null);
                    }
                    prevPacketCount.current = currentCount;
                    setVitals(msg.data);
                } else if (msg.type === "final") {
                    // iOS session ended - stop audio and show report
                    console.log('[LiveVitals] Received FINAL message, stopping audio...');
                    stopAudioRecording();
    ...
    const startAudioRecording = async () => {
        ...
        try {
            ...
            audioWs.current = connectAudioStream((data) => data);

            await audioRecorder.current.start((audioData) => {
                audioWs.current?.send(audioData);
            });

            setIsRecording(true);
            console.log('[LiveVitals] Audio recording started (iOS session began)');
        } catch (error) {
            console.error('[LiveVitals] Failed to start audio:', error);
        }
    };

    const stopAudioRecording = () => {
        console.log('[LiveVitals] stopAudioRecording called, isRecording:', isRecording);

        // Always stop and cleanup, regardless of isRecording state
        ...
        console.log('[LiveVitals] Audio recording stopped (iOS session ended)');
    };
    ...
    // Speech Clarity (default to 100 if not present, or 0 if risk is high but no score? safer to default to 100/N/A)
    // If report exists but no score, it might mean old backend. 
    // If report doesn't exist, show 0 or hidden.
    const speechClarity = report?.speech_clarity_score ?? 0;
            ...
            <main className="flex-1 p-8 lg:p-12 overflow-y-auto z-10 scroll-smooth">
                <header className="flex justify-between items-center mb-12">
                    ...
                    <div className="flex items-center gap-6">
                        {/* Recording Indicator - Shows when iOS session is active and audio is being captured */}
                        {isRecording && (
                            <div className="bg-red-50/80 backdrop-blur-xl border border-red-200 px-5 py-2.5 rounded-full shadow-sm flex items-center gap-3">
                                <span className="w-2.5 h-2.5 rounded-full bg-red-500 animate-pulse shadow-sm shadow-red-400/50"></span>
                                <span className="text-sm font-bold text-red-700 tracking-wide">RECORDING AUDIO</span>
                            </div>
                        )}
                ...
                <div className="grid grid-cols-12 gap-8">
                    ...
                    <div className="col-span-12 lg:col-span-8 space-y-8">
                        ...
                        {/* AI Insight Card - Centered and Horizontal */}
                        <AnimatePresence mode="wait">
                            {report && (
                                <motion.div
                                    key="report"
                                    className="bg-gradient-to-br from-[#4f46e5] to-[#7c3aed] rounded-[2.5rem] p-10 text-white shadow-2xl shadow-indigo-500/30 relative overflow-hidden"
                                ...
                                >
                                    ...
                                    <div className="relative z-10">
                                        <div className="flex items-center gap-3 mb-6 opacity-90">
                                            ...
                                            <h3 className="font-bold text-lg uppercase tracking-widest">AI Insight</h3>
                                        </div>
                                        <p className="text-base leading-relaxed opacity-95 mb-6 font-medium border-l-2 border-white/30 pl-6">{report.summary}</p>
                                        <div className="bg-white/10 backdrop-blur-md rounded-2xl p-6 border border-white/10 hover:bg-white/20 transition-colors">
                                            <p className="text-xs font-bold uppercase tracking-widest opacity-70 mb-3">Recommendation</p>
                                            <p className="text-base font-semibold leading-relaxed">{report.recommendation}</p>
                                        </div>
                                    </div>
                                </motion.div>
                            )}
                    ...
                    <div className="col-span-12 lg:col-span-4 space-y-8">
                        ...
                        >
                            ...
                            <div className="space-y-12 flex-1 relative z-10">
                                ...
                                <div className="relative">
                                    <div className="flex justify-between items-end mb-4">
                                        <span className="font-bold text-gray-600 text-lg">Speech Clarity</span>
                                        <span className="font-extrabold text-blue-600 text-2xl">{Math.round(speechClarity)}%</span>
                                    </div>
                                    <div className="bg-white/40 rounded-3xl p-4 border border-white/50 shadow-sm">
                                        <Gauge
                                            value={speechClarity}
